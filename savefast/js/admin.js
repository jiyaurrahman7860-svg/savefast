...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function applyRolePermissionsUI() {
    ...
    const rolePermissions = {
      'Super Admin': ['overview', 'tools', 'monitoring', 'seo', 'cms', 'blogs', 'ads', 'analytics', 'logs', 'errors', 'security', 'announcements', 'contacts', 'settings'],
      'Admin': ['overview', 'tools', 'monitoring', 'seo', 'cms', 'blogs', 'ads', 'analytics', 'logs', 'errors', 'security', 'announcements', 'contacts', 'settings'],
      'SEO Manager': ['overview', 'seo', 'cms', 'blogs'],
  ...
  async function loadAdsConfiguration() {
    ...
    try {
      ...
      if (doc.exists) {
        ...
        document.getElementById('ad-popunder-toggle').checked = !!data.popunderEnabled;
        document.getElementById('ad-popunder-code').value = data.popunderCode || '';
  ...
  window.saveAdsConfiguration = async function() {
    ...
    try {
      await db.collection('settings').doc('ads').set({
        ...
        popunderEnabled: document.getElementById('ad-popunder-toggle').checked,
        popunderCode: document.getElementById('ad-popunder-code').value,
  ...
  async function loadBlogsList() {
    ...
    try {
      tbody.innerHTML = '<tr><td colspan="5" class="text-center py-4">Fetching blog posts...</td></tr>';
      
      let snapshot = await db.collection('blogs').get();
      
      // Bootstrap from static BLOG_DATA if Firestore is empty
      if (snapshot.empty && typeof BLOG_DATA !== 'undefined') {
        tbody.innerHTML = '<tr><td colspan="5" class="text-center py-4">Bootstrapping from default database...</td></tr>';
        for (const item of BLOG_DATA) {
          await db.collection('blogs').doc(item.slug).set({
            title: item.title,
      ...
      if (snapshot.empty) {
        tbody.innerHTML = '<tr><td colspan="5" class="text-center py-4">No articles found. Click Add to create one.</td></tr>';
        return;
      }
      ...
      snapshot.forEach(doc => {
        const data = doc.data();
        activeBlogs.push({ id: doc.id, ...data });
        ...
        rowsHTML += `
          <tr>
            <td>
              <strong style="color: var(--color-primary);">${sanitize(data.slug)}</strong>
              <div style="font-size: 11px; opacity: 0.8; text-transform: uppercase;">Category: ${sanitize(data.category)}</div>
            </td>
            <td><span style="font-weight: 600;">${sanitize(data.title)}</span></td>
            <td style="font-size: 12px;">${sanitize(data.date)} (${sanitize(data.readTime)})</td>
            <td>
              <button class="status-badge ${statusClass}" style="cursor: pointer; font-size: 10px; border: 1px solid var(--glass-border);" onclick="toggleBlogStatus('${doc.id}')">${statusText}</button>
            </td>
            <td style="display: flex; gap: 8px;">
              <button class="btn btn-glass" style="padding: 6px 12px; font-size: 12px;" onclick="openBlogModal('${doc.id}')">Edit</button>
              <button class="btn btn-glass" style="padding: 6px 12px; font-size: 12px; color: var(--color-error); border-color: rgba(248,113,113,0.2);" onclick="deleteBlogPostItem('${doc.id}')">Delete</button>
            </td>
          </tr>
        `;
      });
      ...
      console.error("Blogs listing error:", err);
      tbody.innerHTML = `<tr><td colspan="5" class="text-center py-4 text-danger">Failed to sync articles: ${err.message}</td></tr>`;
    }
  }

  // Toggle Blog operational ON/OFF status
  window.toggleBlogStatus = async function(id) {
    if (currentUserRole === 'Support Agent') {
      alert("Inquiries agents cannot modify operational states.");
      return;
    }
    const db = window.firebaseAppInstance.db;
    try {
  ...
  window.openBlogModal = function(id = '') {
    const titleInput = document.getElementById('blog-modal-title');
    const overrideForm = document.getElementById('blog-editor-form');
    overrideForm.reset();
    document.getElementById('blog-modal-id').value = id;

    if (id) {
      titleInput.innerText = "Edit Blog Post Settings";
      const blog = activeBlogs.find(b => b.id === id);
      if (blog) {
        document.getElementById('blog-modal-slug').value = blog.slug || '';
        document.getElementById('blog-modal-title-input').value = blog.title || '';
        document.getElementById('blog-modal-description').value = blog.description || '';
        document.getElementById('blog-modal-category').value = blog.category || 'Guides';
        document.getElementById('blog-modal-readtime').value = blog.readTime || '';
        document.getElementById('blog-modal-date').value = blog.date || '';
        document.getElementById('blog-modal-image').value = blog.image || '';
        document.getElementById('blog-modal-active').checked = !!blog.active;
      }
    } else {
      titleInput.innerText = "Add New Blog Post";
      document.getElementById('blog-modal-active').checked = true;
    }
  ...
  if (blogEditorForm) {
    blogEditorForm.addEventListener('submit', async (e) => {
      ...
      const id = document.getElementById('blog-modal-id').value;
      const slug = document.getElementById('blog-modal-slug').value.trim();
      const title = document.getElementById('blog-modal-title-input').value.trim();
      const description = document.getElementById('blog-modal-description').value.trim();
      const category = document.getElementById('blog-modal-category').value;
      const readTime = document.getElementById('blog-modal-readtime').value.trim();
      const date = document.getElementById('blog-modal-date').value.trim();
      const image = document.getElementById('blog-modal-image').value.trim();
      const active = document.getElementById('blog-modal-active').checked;
      ...
      try {
        await db.collection('blogs').doc(slug).set(blogPayload, { merge: true });
        
        if (id && id !== slug) {
          await db.collection('blogs').doc(id).delete();
        }

        alert("Blog Post configuration saved successfully!");
        closeBlogModal();
        loadBlogsList();
      } catch (err) {
  ...
  window.deleteBlogPostItem = async function(id) {
    if (currentUserRole === 'Support Agent') {
      alert("Inquiries agents cannot modify blog files.");
      return;
    }

    if (!confirm("Are you sure you want to delete this blog post? It will be removed from Firestore listing immediately.")) return;
